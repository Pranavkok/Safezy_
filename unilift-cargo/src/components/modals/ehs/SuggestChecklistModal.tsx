...
import { addChecklistSuggestion } from '@/actions/admin/ehs/checklist';
import InputFieldWithLabel from '@/components/inputs-fields/InputFieldWithLabel';

const SuggestChecklistModal = ({
  isOpen,
  setIsOpen
}: {
  isOpen: boolean;
  ...
  const onSubmit = async (data: addSuggestionType) => {
    setLoading(true);
    try {
      const res = await addChecklistSuggestion(data);
      ...
      if (res.success) {
        ...
        toast.error(ERROR_MESSAGES.CHECKLIST_SUGGESTION_NOT_ADDED);
      }
    } catch (error) {
      toast.error(ERROR_MESSAGES.CHECKLIST_SUGGESTION_NOT_ADDED);
    } finally {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      >
        <DialogHeader>
          <DialogTitle>Suggest Checklist Topic</DialogTitle>
        </DialogHeader>

        <form onSubmit={handleSubmit(onSubmit)}>
          <InputFieldWithLabel
            label="Add Checklist Topic"
            type="text"
            required
...
export default SuggestChecklistModal;

...
import { addFirstPrincipleSuggestion } from '@/actions/admin/ehs/first-principles';
...
const SuggestFirstPrincipleModal = ({
  isOpen,
  setIsOpen
}: {
  isOpen: boolean;
  ...
  const onSubmit = async (data: addSuggestionType) => {
    setLoading(true);
    try {
      const res = await addFirstPrincipleSuggestion(data);
      ...
      if (res.success) {
        ...
        toast.error(ERROR_MESSAGES.FIRST_PRINCIPLE_SUGGESTION_NOT_ADDED);
      }
    } catch (error) {
      toast.error(ERROR_MESSAGES.FIRST_PRINCIPLE_SUGGESTION_NOT_ADDED);
    } finally {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      >
        <DialogHeader>
          <DialogTitle>Suggest First Principle</DialogTitle>
        </DialogHeader>

        <form onSubmit={handleSubmit(onSubmit)}>
          <InputFieldWithLabel
            label="Add Principle Title"
            type="text"
            required
...
export default SuggestFirstPrincipleModal;

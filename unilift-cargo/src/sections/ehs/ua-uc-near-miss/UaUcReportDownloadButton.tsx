...
import UaUcReportPdf from './UaUcReportPdf';
import { UaUcNearMissRecord } from '@/types/ehs.types';

// ─── Native helpers (mirrors IncidentReportDownloadButton) ────────────────────
...
async function checkAndRequestStoragePermission(): Promise<boolean> {
  try {
    ...
    // Android fallback — Directory.Data needs no extra permission
...
const UaUcReportDownloadButton = ({ report }: { report: UaUcNearMissRecord }) => {
  ...
  const handleDownload = async (blob: Blob | null) => {
    if (!blob) return;
    const fileName = `${report.report_no.replace(/-/g, '_')}_report.pdf`;
    if (isNativeMobile) {
  ...
  return (
    <div className="flex flex-col gap-2">
      {/* Permission warning (native only) */}
      ...
      {/* Download link */}
      <PDFDownloadLink
        document={<UaUcReportPdf report={report} />}
        fileName={fileName}
...
export default UaUcReportDownloadButton;
